            ...
            <div className="overflow-y-auto max-h-[88vh] scorecard-scroll">
              ...
              <div>
                ...
                <div className="mx-4 border border-t-0 border-border/40 rounded-b-xl bg-card/60 px-0 py-0 mb-4 overflow-hidden">
                  ...
                  <div className="flex justify-between items-center px-4 py-2.5 border-b border-border/15"><span className="text-sm font-bold">Completion Rate</span><span className="text-sm font-black tabular-nums">{d.dcr}</span></div>
                  ...
                  <div className="flex justify-between items-center px-4 py-2.5 border-b border-border/15" style={{ background: 'linear-gradient(270deg, rgba(16,185,129,0.10) 0%, transparent 60%)' }}><span className="text-sm font-bold">Photo-On-Delivery Acceptance</span><span className="text-sm font-black tabular-nums">{podRate}%</span></div>
                  <div className="flex justify-between items-center px-4 py-2.5 border-b border-border/15"><span className="text-sm font-bold">Photo-On-Delivery Rejects</span><span className={cn("text-sm font-black tabular-nums", d.podRejects > 0 && "text-amber-500")}>{d.podRejects}<span className="text-muted-foreground font-normal">/{d.podOpportunities || '—'}</span></span></div>
                  {d.podRejects > 0 && Object.entries(d.podRejectBreakdown).length > 0 && (
                    <>{Object.entries(d.podRejectBreakdown).sort(([,a],[,b]) => (b as number) - (a as number)).map(([reason, count]) => (
                      <div key={reason} className="flex justify-between items-center px-4 py-2 pl-7 border-b border-border/10"><span className="text-sm text-muted-foreground">{reason}</span><span className="text-sm font-black tabular-nums">{count as number}</span></div>
                    ))}</>
                  )}
