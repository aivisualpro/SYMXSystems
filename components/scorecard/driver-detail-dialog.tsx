                    ...
                    {d.profileImage ? (
                      <div className="h-[60px] w-[60px] rounded-full bg-gradient-to-br from-[#1a7a8a] to-[#1a5f6a] p-[2px] shrink-0 shadow-md">
                        <img
                          ...
                          onError={(e) => {
                            // Fall back to initials avatar on broken URL
                            e.currentTarget.parentElement!.innerHTML = `<div class="h-full w-full rounded-full flex items-center justify-center text-lg font-black text-white">${d.name.split(' ').map((n: string) => n[0]).join('').slice(0, 2)}</div>`;
                          }}
                        />
                      </div>
                    ) : (
