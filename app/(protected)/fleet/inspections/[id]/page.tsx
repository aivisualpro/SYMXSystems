...
function InspectionDetailPageContent() {
    ...
    const [compareSource, setCompareSource] = useState<"previous" | "master" | "custom">("previous");
    ...
    const loadCustomCompare = useCallback(async (dateStr: string) => {
        if (!dateStr || !inspection?.vin) return;
        setCompareError(null);
        setCompareLoading(true);
        try {
            const res = await fetch(`/api/fleet/inspections?section=inspection-by-date&vin=${encodeURIComponent(inspection.vin)}&date=${dateStr}&excludeId=${id}`);
            const j = await res.json();
            if (j.inspection) {
                setCustom(j.inspection);
                setCompareSource("custom");
                setCompareMode(true);
                setShowCompareMenu(false);
            } else {
                setCompareError(`No inspection found for this VIN on or before ${dateStr}.`);
            }
        } catch { setCompareError("Failed to load comparison."); }
        finally { setCompareLoading(false); }
    }, [id, inspection?.vin]);
    ...
    const compareData = compareSource === "master" ? master : compareSource === "custom" ? custom : previous;
            ...
            <div className="flex items-center gap-2 relative">
                ...
                <div className="relative">
                    ...
                    >
                        ...
                        {compareMode
                            ? (compareSource === "master" ? "vs Master" : compareSource === "custom" ? "vs Custom Date" : "vs Previous")
                            : "Compare"}
                            ...
                            <div className="absolute right-0 top-full mt-1.5 z-50 w-56 rounded-xl border border-border bg-card shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150">
                                ...
                                <div>
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-3 px-4 py-3 text-sm hover:bg-muted/60 transition-colors text-left ${compareMode && compareSource === "custom" ? "bg-primary/5 text-primary" : "text-foreground"}`}
                                    >
                                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${compareMode && compareSource === "custom" ? "bg-primary/15" : "bg-violet-500/10"}`}>
                                            <IconCalendar size={14} className={compareMode && compareSource === "custom" ? "text-primary" : "text-violet-500"} />
                                        </div>
                                        <div>
                                            <p className="font-medium">Custom Date</p>
                                            <p className="text-[10px] text-muted-foreground">Compare with a specific date</p>
                                        </div>
                                        {compareMode && compareSource === "custom" && (
                                            <span className="ml-auto text-primary text-xs">●</span>
                                        )}
                                    </button>
                                    {showCustomDatePicker && (
                                        <div className="px-4 pb-3 flex items-center gap-2" onClick={e => e.stopPropagation()}>
                                            <input
                                                ...
                                                className="flex-1 px-2.5 py-1.5 rounded-lg bg-muted/50 border border-border text-xs focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20"
                                            />
                                            <button
                                                onClick={() => loadCustomCompare(customDate)}
                                                disabled={!customDate || compareLoading}
                                                className="px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-medium disabled:opacity-40 hover:bg-primary/90 transition-colors"
                                            >
                                                Go
                                            </button>
                                        </div>
                                ...
                                {compareMode && (
                                    <>
                                        <div className="h-px bg-border/60" />
                                        <button
                                            onClick={() => { setCompareMode(false); setShowCompareMenu(false); setShowCustomDatePicker(false); }}
    ...
    }, [setRightContent, compareMode, compareLoading, loadCompare, isStandardPhoto, togglingStandard, toggleStandardPhoto, showCompareMenu, hasStandardPhoto, inspection, openEditModal, compareSource, showCustomDatePicker, customDate, loadCustomCompare]);
    ...
    const compareAfterLabel = compareData
        ? (compareSource === "master" ? `★ Master · ${fmtDateShort(compareData.routeDate)}` : fmtDateShort(compareData.routeDate))
        : "";
                ...
                <div className="space-y-3 sm:space-y-4 pb-6">
                    ...
                    {hasPhotos && (
                        <div>
                            ...
                            {compareMode && compareData && (
                                <div className="rounded-xl sm:rounded-2xl border border-primary/20 bg-primary/5 px-3 sm:px-5 py-2.5 sm:py-3 flex flex-wrap items-center justify-between gap-2 sm:gap-3 text-xs sm:text-sm mb-3 sm:mb-4">
                                    <span className="flex items-center gap-2 sm:gap-3">
                                        {compareSource === "master" ? (
                                            ...
                                            <><IconCalendar size={14} className="text-violet-500" /> Comparing with <strong>{fmtDateShort(compareData.routeDate)}</strong></>
                                        ) : (
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <PhotoCompareTile mode={viewMode} before={photos[0]?.url} after={prevPhotos[0]?.url} beforeLabel={fmtDateShort(inspection.routeDate)} afterLabel={compareAfterLabel} />
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-semibold text-muted-foreground/60 uppercase tracking-wider mb-2">{photos[2]?.label}</p>
                                            <PhotoCompareTile mode={viewMode} before={photos[2]?.url} after={prevPhotos[2]?.url} beforeLabel={fmtDateShort(inspection.routeDate)} afterLabel={compareAfterLabel} />
                                        </div>
                                    </div>
                                    {/* Row 2: Front & Back */}
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <PhotoCompareTile mode={viewMode} before={photos[3]?.url} after={prevPhotos[3]?.url} beforeLabel={fmtDateShort(inspection.routeDate)} afterLabel={compareAfterLabel} />
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-semibold text-muted-foreground/60 uppercase tracking-wider mb-2">{photos[1]?.label}</p>
                                            <PhotoCompareTile mode={viewMode} before={photos[1]?.url} after={prevPhotos[1]?.url} beforeLabel={fmtDateShort(inspection.routeDate)} afterLabel={compareAfterLabel} />
                                        </div>
                                    </div>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            {photos[4]?.url && (
                                                <div>
                                                    ...
                                                    <PhotoCompareTile mode={viewMode} before={photos[4].url} after={prevPhotos[4]?.url} beforeLabel={fmtDateShort(inspection.routeDate)} afterLabel={compareAfterLabel} />
                                                </div>
                                            )}
                                            {photos[5]?.url && (
                                                <div>
                                                    <p className="text-[10px] font-semibold text-muted-foreground/60 uppercase tracking-wider mb-2">{photos[5].label}</p>
                                                    <PhotoCompareTile mode={viewMode} before={photos[5].url} after={prevPhotos[5]?.url} beforeLabel={fmtDateShort(inspection.routeDate)} afterLabel={compareAfterLabel} />
                                                </div>
                                            )}
                                        </div>
                                    )}
