...
/* ─── Tab Button ──────────────────────────────────── */
function TabBtn({ active, label, count, icon: Icon, href }: { active: boolean; label: string; count: number; icon: any; href: string }) {
    return (
        <Link
            href={href}
...
export default function VehicleDetailPage({ params }: { params: Promise<{ id: string; tab: string }> }) {
    ...
    const activeTab: TabId = (VALID_TABS.includes(tab as TabId) ? tab : "overview") as TabId;
    ...
    return (
        <div className="space-y-5 w-full">
            ...
            <div className="flex items-center gap-1 p-1 rounded-xl bg-muted/50 border border-border overflow-x-auto">
                {tabs.map((t) => (
                    <TabBtn key={t.id} active={activeTab === t.id} label={t.label} count={t.count} icon={t.icon} href={`/fleet/vehicles/${id}/${t.id}`} />
                ))}
            </div>
...
/* ────────────────────────────────────────────────────
   TAB: Inspections
   ──────────────────────────────────────────────────── */
function InspectionsTab({ vehicleId, inspections }: { vehicleId: string; inspections: any[] }) {
    const router = useRouter();
    ...
    // Scroll to + animate the highlighted row, then clear after 3s
    useEffect(() => {
    ...
    // returnTo tells the inspection detail page where the back button should go
    const returnTo = `/fleet/vehicles/${vehicleId}/inspections`;

    return (
        <div className="overflow-auto rounded-xl border border-border/60 shadow-sm bg-card">
            <style>{`
                @keyframes inspect-highlight {
                    0%   { background-color: hsl(var(--primary) / 0.18); box-shadow: inset 4px 0 0 hsl(var(--primary)); }
                    70%  { background-color: hsl(var(--primary) / 0.08); box-shadow: inset 4px 0 0 hsl(var(--primary) / 0.4); }
                    100% { background-color: transparent; box-shadow: none; }
                }
                .row-highlight { animation: inspect-highlight 3s ease-out forwards; }
            `}</style>
            <table className="w-full border-collapse">
                ...
                <tbody className="divide-y divide-border/40">
                    {inspections.map((r: any, idx: number) => {
                        const isHighlighted = highlighted === r._id;
                        return (
                            <tr
                                key={r._id}
                                ref={isHighlighted ? highlightRef : undefined}
                                onClick={() => router.push(
                                    `/fleet/vehicles/${vehicleId}/inspections/${r._id}?returnTo=${encodeURIComponent(returnTo + "?highlight=" + r._id)}`
                                )}
                                className={`relative group cursor-pointer transition-all duration-150 hover:bg-primary/[0.035] hover:shadow-[inset_3px_0_0_hsl(var(--primary))] ${idx % 2 === 0 ? "bg-transparent" : "bg-muted/[0.015]"} ${isHighlighted ? "row-highlight" : ""}`}
                            >
                                {cols.map(col => (
                                    <td key={col.key}
