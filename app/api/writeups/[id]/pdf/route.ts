...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const buffer = generateCoachingPdfBuffer({
      ...
      priorDates: (w.priorWriteups || []).map((p: any) => ({ date: p.incidentDate, warningLevel: p.warningLevel, label: p.subCategory || p.categoryLabel })),
      priorVerbalCoachingDates: (w.priorVerbalCoachings || []).map((v: any) => v.coachingDate),
      totalInfractionCount: w.totalInfractionCount,
      categoryBreakdown: w.categoryBreakdown,
      lookbackDaysUsed: w.lookbackDaysUsed,
